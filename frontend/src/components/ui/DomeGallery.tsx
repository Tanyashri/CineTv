...
const DEFAULT_IMAGES: ImageItem[] = [
  {
    src: 'https://images.unsplash.com/photo-1755331039789-7e5680e26e8f?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA==',
    alt: 'Abstract art',
  },
  {
    src: 'https://images.unsplash.com/photo-1755569309049-98410b94f66d?q=80&w=772&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA==',
    alt: 'Modern sculpture',
  },
  {
    src: 'https://images.unsplash.com/photo-1755497595318-7e5e3523854f?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA==',
    alt: 'Digital artwork',
  },
];
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  for (let i = 1; i < usedImages.length; i++) {
    const uPrev = usedImages[i - 1];
    const uCurr = usedImages[i];
    if (uPrev && uCurr && uCurr.src === uPrev.src) {
      for (let j = i + 1; j < usedImages.length; j++) {
        const uTarget = usedImages[j];
        if (uTarget && uTarget.src !== uCurr.src) {
...
export function DomeGallery({
  ...
  // Time-delta based auto-rotation when user is not interacting
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // Only auto-rotate if not dragging, not focused on an item, and not opening an item
      if (!draggingRef.current && !focusedElRef.current && !openingRef.current && !inertiaRAF.current) {
        const speed = autoRotateSpeed * (delta / 16.666); // normalize speed to 60fps
